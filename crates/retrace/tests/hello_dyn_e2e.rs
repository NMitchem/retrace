...
// `--ignored`.
//
// The libsystem_secinit task_info(TASK_AUDIT_TOKEN) wall FELL in M2-taskinfo — and it was the LAST wall on
// the hello_dyn path, so THE M2 HEADLINE GATE IS NOW GREEN (the #[ignore] is gone; this test runs in the
// default gate). retrace's MIG router (machmsg::route) now FORWARDS task_info (msgh_id 3405) via
// FORWARD_ALLOWLIST: on record it issues the real task_info trap against retrace's OWN task
// (forward_and_diff captures the audit-token reply bytes); on replay it applies the recorded writes verbatim.
// This is the forward-and-record posture (like task_self's port name and getentropy), because the audit token
// holds host process identity (pid/asid/pidversion vary run-to-run) and is nondeterministic — NOT
// synthesize-and-byte-compare. Forwarding is SAFE here precisely because task_info(TASK_AUDIT_TOKEN) returns
// read-only out-of-line data with NO port rights (unlike 3409 task_get_special_port / 3410
// task_set_special_port, which carry port descriptors and so had to be minted/synthesized to keep the guest's
// IPC space coherent). With the token in hand, libsystem_secinit's _libsecinit_appsandbox_check proceeds,
// _libsecinit_initializer completes, and — with no further libSystem sub-initializer wall — dyld's
// findAndRunAllInitializers returns; control reaches the program's main and hello_dyn runs to completion:
// write(1, "hi\n", 3) (trap 4, pc 0x1804af834) then exit(0) (trap 1, pc 0x1804b3584). Record produces exit 0
// / stdout "hi\n" with 3405 forwarded (no RECORD ERROR); replay is byte-identical (exit 0 / "hi\n", empty
// stderr, zero divergence), verified twice (double-replay). The libsecinit sandbox check the M2-setport
// re-park warned MIGHT surface a further entitlement/sandbox query did NOT — xpc_copy_entitlements_for_self
// returned from the forwarded token alone. Deferred debt unchanged: the frozen HOST commpage still carries
// 12-CPU/2-cluster COUNTS (harmless once the cpu/cluster index is pinned to 0; single-vCPU commpage synthesis
// is the hygiene follow-up), and the XPC send / dispatch-mach subsystem was never exercised (no mach_msg2
// ever targets the minted bootstrap port 0x1003; no bootstrap_look_up appears). See
