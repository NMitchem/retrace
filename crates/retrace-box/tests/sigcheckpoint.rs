...
//
// M16 split the blocked mask and the alternate stack off SigTable and onto Thread (they are
// per-thread; disposition stays process-wide) — `BoxState` already carries `threads` wholesale, so
// this test's mask/altstack assertions moved to `r.threads().mask_of(0)`/`altstack_of(0)` rather
// than being deleted, which is what proves the split didn't quietly drop the carriage.
