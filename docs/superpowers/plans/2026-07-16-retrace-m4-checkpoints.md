...
- **`checkpoint()`/`from_checkpoint()` correctness lives or dies on the field list.** Task 2's round-trip test is the backstop — if you add any NEW field to `Box_` in a future change, it must be added to `BoxState` too, or this test's `dbg_internal_state`/`dbg_fp_regs`/`diff_memory` trio will not catch it (they only compare what they know to compare). This is a standing invariant, not a one-time check.
