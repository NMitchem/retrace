...
- [ ] **Step 3: Implement** the signing stub + `sign_slots` (save vCPU state → load slot table into guest scratch → run stub that `pac{ia,da}`s each and `hvc`s → read back → restore state). Bounded-timeout the run.
