...
- **Slide is pinned to 0** by making `shared_region_check_np` (#294) return the UNSLID base `0x180000000` — dyld then infers slide 0 (`base − sharedRegionStart == 0`) and computes all cache addresses at the unslid VAs, which is exactly what the pager maps. #536 just returns success (0) + `install_cache_pager`; the slide consistency comes from #294.
- **Faulting-IPA routing:** a stage-2 abort on an unmapped cache page arrives as a `Stop::Other` data/instruction abort; the faulting guest-physical address is in the exit's `virtual_address` (the M2 box already surfaces ESR/FAR on `Stop::Other` — surface the fault IPA too if not already, e.g. a `Box_` accessor). The dispatch checks `CacheMeta.window()` contains the IPA → `page_in_cache(ipa)` → re-run the faulting instruction (it now hits a mapped page).
- **`page_in_cache` order (DATA):** `pread` the pristine page from the subcache file into an anon backing → `walk_page` (applies regular rebases in place, returns auth slots) → `sign_slots(auth_slots)` (in-guest) → write each signed pointer into the page at its slot offset → `hv_vm_map` the anon page at the cache IPA (stage-2). **TEXT** (no slide-info): `pread` → map + `set_region_exec` (RO+exec `ATTR_CODE`, no fixups). Map DATA with the default RW+non-exec stage-1 (the identity block map already covers the cache window); note `__DATA_CONST` (initProt `r--`) may want RO — a T6 refinement if dyld faults writing it.
- **Record/replay symmetry:** `page_in_cache` is a pure deterministic function of (file bytes, slide 0, fixed keys), so route cache faults to it on BOTH record and replay; cache page contents are NEVER written to the trace (regenerated). #294/#536 are handled identically on both.

- [ ] **Step 1: Failing test** — focused on the pager MECHANISM (not the full dyld run — that's T6): install the pager, directly `page_in_cache(ipa)` for the page containing the spike's worked-example DATA slot, then have the guest load that re-signed pointer and `autda` it — assert it authenticates (no FPAC EC=0x1C). Also assert a TEXT page faults in as executable. Run under a bounded timeout.
- [ ] **Step 2: Run → FAIL** (pager missing).
- [ ] **Step 3: Implement** the pager + dispatch routing per the integration points above; regenerate identically on record and replay (no cache bytes in the trace).
